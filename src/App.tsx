...
function App() {
  ...
  // 💡 NEW: Save last brightness before turning off
  ...
  // 💡 NEW: Lights ON handler
  const handleLightsOn = () => {
    console.log("\n💡 LIGHTS ON pressed");
    console.log(`📍 Current brightness: ${lightsBrightness}%`);
    console.log(`💾 Last saved brightness: ${lastBrightnessRef.current}%`);
    ...
    if (lightsBrightness === 0) {
      ...
      console.log(`✅ Setting brightness to: ${targetBrightness}%\n`);
    } else {
      console.log(`⚠️ Lights already on (${lightsBrightness}%) - No change\n`);
    }
  };

  // 💡 NEW: Lights OFF handler
  const handleLightsOff = () => {
    ...
    if (lightsBrightness > 0) {
      ...
      console.log(`💾 Saved brightness: ${lightsBrightness}%`);
      setLightsBrightness(0);
      console.log(`✅ Lights turned OFF\n`);
    } else {
      console.log(`⚠️ Lights already off - No change\n`);
    }
  };

  // 💡 NEW: Update lastBrightnessRef when slider is manually changed
  useEffect(() => {
    if (lightsBrightness > 0) {
      lastBrightnessRef.current = lightsBrightness;
    }
    ...
    >
      ...
      >
        <ControlPanel
          ...
          onLightsOn={handleLightsOn} // 💡 NEW
          onLightsOff={handleLightsOff} // 💡 NEW
